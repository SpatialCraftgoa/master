...
const measureIcon = document.getElementById("measureIconUnique123");
...
// Reset Button Clears All Measurements (even ongoing)
resetBtn.addEventListener("click", function () {
  if (draw) {
    map.removeInteraction(draw); // stop the current drawing interaction
    draw = null;
  }
  source.clear();
  map.getOverlays().clear();
  ...
  createHelpTooltip();
  if (measureActive) {
    addInteraction(); // re-enable drawing if measuring is active
  }
});
